...
List<Map<String, dynamic>> aggregate<K>(List<Map<String, dynamic>> xs,
    List<String> groupBy, List<String> variables, K Function(Iterable<K>) f) {
  ...
  if (groupBy.isEmpty) {
    ...
    for (var variable in variables) {
      row[variable] = f(xs.map((e) => e[variable]));
    }
    res.add(row);
  } else {
...
Map<Map<String, dynamic>, List<Map<String, dynamic>>> groupByColumns(
    List<Map<String, dynamic>> xs, List<String> columns) {
  ...
  var f = (Map<String, dynamic> e) =>
      {for (var column in columns) column: e[column]};
...
Map<dynamic, List<int>> _indexByColumns(
    Iterable<Map<String, dynamic>> xs, List<String> columns) {
  var f = (Map<String, dynamic> e) =>
      {for (var column in columns) column: e[column]};
